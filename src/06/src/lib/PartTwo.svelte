...
function computeGuard() {
	...
	const guardPos: Position = findGuard();
	...
	do {
		...
		getNextCell(guardPos, direction).x >= 0 &&
		getNextCell(guardPos, direction).x < labMap.length &&
		getNextCell(guardPos, direction).y >= 0 &&
		getNextCell(guardPos, direction).y < labMap[0].length
	);
